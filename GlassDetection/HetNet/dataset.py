...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(self.cfg.datapath + '/' + self.cfg.mode + '/image/' + name + '.jpg')[:, :, ::-1].astype(
            np.float32)
...
class InferenceData(Dataset):
    def __init__(self, cfg, images_list):
        self.normalize = Normalize(mean=cfg.mean, std=cfg.std)
        self.resize = Resize(352, 352)
        self.totensor = ToTensor()
    ...
    def __getitem__(self, idx):
        image = cv2.imread(self.images_list[idx])[:, :, ::-1].astype(np.float32)
